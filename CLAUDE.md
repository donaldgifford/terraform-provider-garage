...
**Current state (RFC-0001 Phases 1-2 shipped):** Phase 1 merged via [PR #6](https://github.com/donaldgifford/terraform-provider-garage/pull/6). Phase 2 (the `garage_bucket` resource) shipped on `feat/design-0002-phase-2-bucket-resource` per all 8 phases of [IMPL-0002](docs/impl/0002-phase-2-garagebucket-resource-client-wrapper-crud-acceptance.md). `main.go` at repo root wired to `providerserver.Serve`. `internal/provider/provider.go` registers the `GarageProvider` with `endpoint` / `token` + three optional S3 attrs (`s3_endpoint` / `s3_access_key` / `s3_secret_key`); all six have env fallback. `Configure` builds a `*client.Client`, bundles it with the resolved S3 fields into a `*client.ProviderData`, and plumbs that into both `ResourceData` and `DataSourceData`. `internal/client/openapi/generated.go` is the `oapi-codegen` output of the Garage admin v2 OpenAPI spec, generated via `just generate` (upstream is 3.1.0; `tools/cmd/spec-downgrade/` shims it to 3.0.3 for `kin-openapi`'s parser). `internal/client/client.go` wraps the generated client with bearer auth, typed sentinel errors, idempotent-verb retries, and the seven admin operations the bucket lifecycle needs (`CreateBucket`, `GetBucket`, `UpdateBucket`, `DeleteBucket`, `AddBucketAlias`, `RemoveBucketAlias`, `AllowBucketKey`). `internal/resources/bucket/` implements the `garage_bucket` resource: Create + Read + Update (alias diff with adds-before-removes; quota state transitions) + Delete (with `force_destroy` via `github.com/aws/aws-sdk-go-v2/service/s3` to empty the bucket through the S3 data plane before the admin DeleteBucket) + ImportState (bare-id passthrough). 11 acceptance tests pass against `dxflrs/garage:v2.3.0` covering alias updates, quota states, drift detection, the three force_destroy paths, import round-trip, parallel safety, and the literal-zero quota distinction. `internal/datasources/clusterinfo/` carries the only data source so far (`garage_cluster_info`). RFC-0001 Phases 3-6 (`garage_key`, `garage_bucket_key`, `garage_bucket_alias`, additional data sources) still ahead.
