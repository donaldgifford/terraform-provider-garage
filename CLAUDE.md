...
**Current state (RFC-0001 Phase 1 shipped; Phase 2 mid-flight):** Phase 1 merged via [PR #6](https://github.com/donaldgifford/terraform-provider-garage/pull/6). `main.go` at repo root wired to `providerserver.Serve` (with `-debug` flag). `internal/provider/provider.go` registers the `GarageProvider` with its `endpoint` / `token` schema + env fallback (`GARAGE_ENDPOINT` / `GARAGE_TOKEN`); `Configure` builds a `*client.Client` and plumbs it into both `ResourceData` and `DataSourceData`. `internal/client/openapi/generated.go` is the `oapi-codegen` output of the Garage admin v2 OpenAPI spec, generated via `just generate`. The upstream spec is OpenAPI 3.1.0; `tools/cmd/spec-downgrade/` rewrites it to 3.0.3-compatible JSON because `kin-openapi` (oapi-codegen's parser) is 3.0-only. Both `garage-admin-v2.json` (raw) and `garage-admin-v2.openapi30.json` (downgraded) are committed. `internal/client/client.go` wraps the generated client with bearer auth, typed sentinel errors, idempotent-verb retries, and the six bucket CRUD methods (`CreateBucket`, `GetBucket`, `UpdateBucket`, `DeleteBucket`, `AddBucketAlias`, `RemoveBucketAlias`). RFC-0001 Phase 2 (the `garage_bucket` resource) is tracked in [IMPL-0002](docs/impl/0002-phase-2-garagebucket-resource-client-wrapper-crud-acceptance.md); design rationale lives in [DESIGN-0002](docs/design/0002-phase-2-implementation-garagebucket-resource.md). IMPL-0002 progress: Phases 1-4 shipped — the resource is registered (`internal/resources/bucket/`) with Create + Read + minimal Delete plus two passing acceptance tests; Phases 5-8 (Update, force_destroy with `aws-sdk-go-v2`, ImportState, polish) still ahead. `internal/datasources/clusterinfo/` carries the only registered data source so far (`garage_cluster_info`).
